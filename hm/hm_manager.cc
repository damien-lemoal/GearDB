...
namespace leveldb{
  ...
  unsigned int HMManager::set_first_zonenum()
  {
    ...
    for (i = 0; i < zonenum_; i++){
      if (zone_[i].zbz_type == ZBC_ZT_SEQUENTIAL_REQ ||
	  zone_[i].zbz_type == ZBC_ZT_SEQUENTIAL_PREF)
	return i;
    }
  ...
  {
    for (ssize_t i = first_zonenum_;i < zonenum_; i++) {  //Traverse from the first sequential write zone
      if (bitmap_->get(i) == 0) {
	unsigned int num = 1;
  ...
  void HMManager::hm_free_zone(uint64_t zone)
  {
    ...
    if (ret != 0){
      MyLog("reset zone:%llu faild! error:%ld\n", zone, ret);
    }
    bitmap_->clr(zone);
  ...
  ssize_t HMManager::hm_write(int level,uint64_t filenum,const void *buf,uint64_t count)
  {
    ...
    ssize_t ret;
    uint64_t write_time_begin = env->NowMicros();
    ...
    write_time += env->NowMicros() - write_time_begin;
  ...
  ssize_t HMManager::hm_read(uint64_t filenum, void *buf, uint64_t count, uint64_t offset)
  {
    ...
    uint64_t de_ofst;
    ssize_t ret;
    uint64_t read_time_begin = env->NowMicros();
    ...
    // Align with logical block
    sector_count = ((count + de_ofst) % 512) ? (count + de_ofst) / 512 + 1 : (count + de_ofst) / 512;
    ...
    free(r_buf);
    if (ret <= 0) {
      printf("error:%ld hm_read failed!\n",ret);
      return -1;
    }

    read_time += env->NowMicros() - read_time_begin;
  ...
  void HMManager::move_zone(uint64_t filenum)
  {
    ...
    for(i = 0; i < zone_info_[level + 1].size(); i++)
  ...
  ssize_t HMManager::adjust_com_window_num(int level)
  {
    ...
    case 1:
    case 2:
      //1,2 level's compaction window number is all the level
      return zone_info_[level].size();
    case 3:
    case 4:
    case 5:
    case 6:
    case 7:
      //other level compaction window number is 1/COM_WINDOW_SCALE
      return zone_info_[level].size()/COM_WINDOW_SCALE;
    default:
      return 0;
    }
  }
  ...
  void HMManager::get_com_window_table(int level,
  ...
  void HMManager::get_per_level_info()
  {
	...
	percent = 100.0 * table_size /
	  ((zone_num - 1) * 256.0 * 1024 * 1024 + (zone_[zone_id].zbz_write_pointer - zone_[zone_id].zbz_start) * 512.0);
      }
  ...
  void HMManager::get_all_info()
  {
    ...
    MyLog("read_time:%.1f s write_time:%.1f s read:%.1f MB/s write:%.1f MB/s\n",
	  ...
	  (kv_read_sector / 2048.0) / (read_time * 1e-6),
	  (kv_store_sector / 2048.0) / (write_time * 1e-6));
    get_valid_info();
    MyLog("\n");
  }
  ...
  {
    MyLog6("\nnum:%d table_size:%ld MB kv_read_sector:%ld MB kv_store_sector:"
	   "%ld MB zone_num:%ld max_zone_num:%ld move_size:%ld MB\n",
	   num, all_table_size / (1024 * 1024),
	   kv_read_sector / 2048, kv_store_sector / 2048, get_zone_num(),
	   max_zone_num, move_file_size / (1024 * 1024));
    MyLog6("read_time:%.1f s write_time:%.1f s read:%.1f MB/s write:%.1f MB/s\n",
	   read_time * 1e-6, write_time * 1e-6,
	   (kv_read_sector / 2048.0) / (read_time * 1e-6),
	   (kv_store_sector / 2048.0) / (write_time * 1e-6));
  ...
  void HMManager::get_valid_all_data(int num)
  {
    ...
    MyLog3("write_zone:%ld delete_zone_num:%ld max_zone_num:%ld table_num:%ld table_size:%ld MB\n",
	   get_zone_num(), delete_zone_num, max_zone_num,
	   table_map_.size(), all_table_size / 1048576);
    uint64_t table_num;
    uint64_t table_size;
    int zone_num=0;
    uint64_t zone_id;
